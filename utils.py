...
# filtering out input for prediction, e.g. for prediction of a third word only first and second words should be used
def create_look_ahead_mask(size):
    mask = 1 - tf.linalg.band_part(tf.ones((size, size)), -1, 0)
    return mask  # (seq_len, seq_len)


def filter_max_length(x, y, max_length=constants.MAX_LENGTH):
